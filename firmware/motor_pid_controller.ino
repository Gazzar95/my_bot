...
const bool MOTOR_INVERT_RIGHT = true;
...
void ISR_enc_r()
{
  if (digitalRead(ENC_R_B)) enc_r++;
  else enc_r--;
}

void ISR_enc_l()
{
  if (digitalRead(ENC_L_B)) enc_l++;
  else enc_l--;
}
...
const float INTEG_MAX = 800.0f;

/* ----- Start assist ----- */
const int START_PWM_R = 35;
const int START_PWM_L = 35;
const float START_VEL_THRESH_RAD_S = 0.3f;
...
static inline void driveMotor(int pwmPin, int dirPin, float pwmSigned, bool invertMotor)
{
  ...
  if (invertMotor) forward = !forward;
...
void handleProtocolLine(const String &line)
{
  ...
  float left = 0.0f;
  float right = 0.0f;
  int parsed = sscanf(line.c_str(), "CMD %f %f", &left, &right);
  if (parsed == 2)
  {
...
void loop()
{
  ...
  if (ENCODER_SIGN_RIGHT) vel_r = -vel_r;
  if (ENCODER_SIGN_LEFT) vel_l = -vel_l;
  ...
  if (ENCODER_SIGN_RIGHT) pos_r_rad = -pos_r_rad;
  if (ENCODER_SIGN_LEFT) pos_l_rad = -pos_l_rad;
  ...
  {
    if (pwm_r > 0) pwm_r = max(pwm_r, (float)START_PWM_R);
    if (pwm_r < 0) pwm_r = min(pwm_r, -(float)START_PWM_R);
  }
  if (fabs(target_l) > 0.1f && fabs(vel_l_f) < START_VEL_THRESH_RAD_S)
  {
    if (pwm_l > 0) pwm_l = max(pwm_l, (float)START_PWM_L);
    if (pwm_l < 0) pwm_l = min(pwm_l, -(float)START_PWM_L);
  }
